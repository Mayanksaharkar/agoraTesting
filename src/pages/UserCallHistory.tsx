...
import { Phone, Video, Clock, Calendar, Filter, ChevronLeft, ChevronRight, Search } from 'lucide-react';
...
export default function UserCallHistory() {
  ...
  const fetchCallHistory = async () => {
    ...
    try {
      ...
      const data = await userApi.getCallHistory(params);
      setCalls(data.calls || []);
  ...
  const filteredCalls = calls.filter((call) => {
    ...
    const astrologerName = call.astrologerId?.personalDetails?.pseudonym || 
                          call.astrologerId?.personalDetails?.name || '';
    return astrologerName.toLowerCase().includes(filters.search.toLowerCase());
  });
            ...
            <div className="space-y-4">
              {filteredCalls.map((call) => (
                <Card
                  ...
                  onClick={() => navigate(`/user/calls/${call._id}/details`)}
                >
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-semibold flex-shrink-0">
                        {call.astrologerId?.personalDetails?.profileImage ? (
                          <img
                            src={call.astrologerId.personalDetails.profileImage}
                            alt="Astrologer"
                            className="w-full h-full rounded-full object-cover"
                          />
                        ) : (
                          (call.astrologerId?.personalDetails?.pseudonym?.[0] ||
                            call.astrologerId?.personalDetails?.name?.[0] ||
                            'A')
                        )}
                      </div>
